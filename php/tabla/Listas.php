 <?php 
	...
	$depa=$_POST['depa'];
	$anio=$_POST['anio'];
	$query="SELECT * FROM listas where unidad='$unidad' and status='Espera' and departamento='$depa' and anio='$anio'";
	$res=$db->query($query);
	$total=$res->num_rows;
	$paginas=ceil($total/ 20);
	// echo $paginas;
	$query="SELECT * FROM listas where unidad='$unidad' and status='Espera' and departamento='$depa' and anio='$anio' LIMIT $limite, 20";
	$res=$db->query($query);
?>
